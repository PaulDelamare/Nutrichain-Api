...
export const generateInvitation = async (req: Request, res: Response) => {
    try {
        ...
        const inviterEmail = (req as any).user?.email || "Administrateur";

        if (!inviterId) {
             res.status(401).json({ status: 401, message: "Utilisateur non identifié pour créer l'invitation." });
             return;
        }
        ...
        if (!targetOrg) {
             res.status(404).json({ status: 404, message: "La zone (Organisation) demandée n'existe pas." });
             return;
        }
