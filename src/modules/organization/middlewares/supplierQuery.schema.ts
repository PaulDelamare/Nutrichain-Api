...
/**
 * Query params de `GET /organization/suppliers`.
 *
 * Pagination OPTIONNELLE : la page Configuration envoie `page` et reçoit `{ data, pagination }` ;
 * les sélecteurs (réception…) n'envoient rien et gardent le tableau simple. Le contrôleur bascule
 * sur la présence de `page`. `includeArchived` reste pour le chemin non paginé.
 *
 * Filtres du chemin paginé : `nom` (recherche libre sur `nom_ferme`) et `statut`
 ...
 */
export const supplierQuerySchema = vine.object({
...
export type SupplierQuery = Infer<typeof supplierQuerySchema>;

export const SUPPLIER_PAGE_DEFAULTS = { page: 1, limit: DEFAULT_PAGE_SIZE } as const;
