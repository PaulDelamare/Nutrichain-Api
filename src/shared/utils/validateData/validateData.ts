 ...
 */
const localizeErrorMessages = (
  error: unknown
): { status: number; error: { field: string; message: string; rule: string }[] } => {
  if (error instanceof errors.E_VALIDATION_ERROR) {
    const translatedMessages = error.messages.map((msg: ValidationMessage) => {
      ...
      if (typeof customMessage === 'function') {
        ...
        translatedMessage = metaValue !== undefined ? customMessage({ data: metaValue }) : message;
      } else {
