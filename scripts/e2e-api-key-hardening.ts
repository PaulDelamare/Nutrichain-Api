...
async function main() {
  ...
  const annuaire = await fetch(`${API_BASE}/api/organization/members`, { headers: cle });
  ...
  const catalogue = await fetch(`${API_BASE}/api/connectors/exports/events`, { headers: cle });
